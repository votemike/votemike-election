<?php namespace Votemike\DbHelper; //@TODO change to DbHelper namespace
...
class DbHelperController extends Controller {
	...
	{
		return view('dbhelper.home', ['tables' => $this->getTableSuggestions()]);
	}
	...
	public function getTableSuggestions($table = null) {
		...
		foreach($results as $result) {
			$tableCollection->addColumnToTable($result->TABLE_NAME, $result->COLUMN_NAME, $result->COLUMN_TYPE, $result->DATA_TYPE);
		}
	...
	private function getDatabaseColumns($table = null) {
		...
		if(is_null($table)) {
			return DB::table('information_schema.columns')->where('TABLE_SCHEMA', DB::connection()->getDatabaseName())->get(); //All columns in current DB
		}
		return DB::table('information_schema.columns')->where('TABLE_SCHEMA', DB::connection()->getDatabaseName())->where('TABLE_NAME', $table)->get(); //All columns in table
	}
}
